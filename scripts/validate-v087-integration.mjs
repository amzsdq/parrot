...
for (const name of ['manifest.json','route-state.js','route-queue.js','background.js','content.js','signal-scanner.js','chatgpt-adapter.js','prompt-compose.js','repeat-policy.js','popup.js']) if (!existsSync(join(ext,name))) fail(`missing extension/${name}`);
if (!failures) {
  ...
  const scripts=manifest.content_scripts?.flatMap((e)=>e.js||[])||[]; const routeIndex=scripts.indexOf('route-state.js'); const scannerIndex=scripts.indexOf('signal-scanner.js'); const promptIndex=scripts.indexOf('prompt-compose.js'); const repeatIndex=scripts.indexOf('repeat-policy.js'); const contentIndex=scripts.indexOf('content.js');
  ...
  promptIndex>=0&&promptIndex<contentIndex?pass('prompt composer loads before content runner'):fail('prompt-compose.js must load before content.js');
  repeatIndex>=0&&repeatIndex<contentIndex?pass('repeat policy loads before content runner'):fail('repeat-policy.js must load before content.js');
