...
for (const name of ['manifest.json', 'route-state.js', 'route-queue.js', 'background.js', 'content.js', 'chatgpt-adapter.js']) {
...
if (!failures) {
  ...
  if (/importScripts\([^)]*['"]route-state\.js['"][^)]*\)/.test(background)) pass('background imports canonical route-state.js');
  else fail('background must import route-state.js with importScripts');
  if (/importScripts\([^)]*['"]route-queue\.js['"][^)]*\)/.test(background)) pass('background imports canonical route-queue.js');
  else fail('background must import route-queue.js with importScripts');
  ...
  if (background.includes('PARROT_ROUTE_RETRY') && background.includes('PARROT_ROUTE_RESOLVE')) pass('background exposes explicit ambiguity actions');
  else fail('background ambiguity actions missing');
  ...
  if (/await\s+persistAmbiguity\(receipt\)[\s\S]*sendMessage\(\{\s*type:\s*['"]PARROT_ROUTE_AMBIGUOUS['"]/.test(content)) pass('content persists ambiguity before transient notification');
  else fail('ambiguity must persist before background notification');
