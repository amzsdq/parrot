...
for path in "$OUT" "$OUT.sha256" "$OUT.files.txt" "$OUT.provenance.txt" "$EVIDENCE"; do
...
grep -Fxq "live_result_sha256=$EVIDENCE_SHA" "$OUT.provenance.txt" || { echo "FAIL: provenance is not bound to durable live evidence before publish" >&2; exit 1; }
...
printf 'release_ready=YES\nartifact_sha256=%s\nchecksum_sha256=%s\nfile_list_sha256=%s\nprovenance_sha256=%s\nlive_evidence_sha256=%s\n' \
  ...
  "$(sha256sum "$OUT.provenance.txt" | awk '{print $1}')" \
  "$EVIDENCE_SHA" > "$TMP_READY"
mv -- "$TMP_READY" "$READY"
trap - EXIT
