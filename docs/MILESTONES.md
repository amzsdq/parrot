...
- Release builder hardening commit `dd128d09ea3476775b5838ee0ca213d89ec5a69f` additionally rejects staged, unstaged, or untracked `extension/` changes before packaging, restricts release output to `.zip`, and clears stale provenance before rebuilding. This prevents packaging dirty product bytes under a clean recorded release-repository SHA. Route State Contract run `35984699779` is the exact-head verification run and was still in progress when this milestone entry was written; do not treat this hardening as verified until it succeeds.
