...
- Live-result verifier now checks case-row evidence plus decision-summary consistency, UTC decision time, and explicit limitation text when S9/S10 is NOT_OBSERVED. Initial CI exposed that the verifier self-test fixture did not include the newly required Decision section; corrected exact-head `49a9da4fecdc9afa2fd99bc45fba311cebe3246b` passed Route State Contract run `35983716086`.
- Final release builder previously passed current repository HEAD as expected M6 candidate SHA, which would incorrectly reject valid live evidence after docs/release-tooling-only commits (or tempt unsafe rebinding). It now verifies live evidence against exact candidate `f51e4ba53753dade3bd3f9a64e2b3c50ca05d691`, records release-repository SHA separately, and fail-closes if current `extension/` differs from candidate tree. Any extension change after candidate creation requires a new candidate and fresh M6.
- Independent Git compare from candidate `f51e4ba...` through checkpoint head `6e5eae4a51ff9618656955c3afd4a50769547206` reported 38 commits ahead with changed files only in workflows/README/docs/scripts and **no `extension/` file changes**, confirming the currently prepared release tree still matches the M6 candidate product bytes.
- README and release-notes draft now document the candidate-tree identity rule; final provenance sidecar records candidate SHA + release-repository SHA.
