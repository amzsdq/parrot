...
## Anti-rollback control-plane design audit

A freshness root must be both independent of the candidate publication and updateable without letting the release signer certify its own authority. In the current GitHub-centered architecture, none of the obvious repository-local shortcuts is sufficient by itself:

- A mutable `latest` file on `main` is durable and easy to consume, but if the same release workflow can write it or a consumer simply trusts whatever current branch bytes say, publication and freshness authorization collapse into one authority. Branch protection/rulesets can make this useful only when policy-state changes require a distinct reviewed control path that the release signer cannot bypass.
- A release tag is a useful immutable publication identifier after immutable releases are enabled, but it is not a monotonic latest pointer. GitHub explicitly permits the `latest` release designation to change even for immutable releases, and ordinary tags can be moved unless separately protected. An immutable release protects that release's tag/assets from mutation; it does not prove that no newer authorized release exists.
- A signer-maintained counter is circular. The same compromised/stale signer that presents generation N can replay N-1 or write whichever generation it wants unless consumers already possess an independently trusted minimum/current value.

The minimal future design should therefore treat freshness as **release-policy state controlled by a distinct authorization boundary**, even if the bytes are stored in the same GitHub repository. A practical GitHub-native candidate after M6 PASS is a dedicated policy path containing exactly `generation + publication_record_sha256`, changed only through a protected, reviewed control path whose bypass principals exclude the release signer token/workflow. Consumers that assert `latest` trust that policy state (or a separately provisioned digest of it), then verify the presented publication against it. The publication workflow may prepare/propose the next policy value but must not be able to approve or directly lower/replace the authoritative value.

Rotation must be compare-and-swap in semantics: the authorized policy transition names the exact prior generation/digest and the exact next generation/digest, rejects stale-base updates, and advances generation exactly once. A normal release transition is `N -> N+1`; no operation rewrites generation N in place. Repository rulesets can restrict creation/update/deletion and require review/status policy on the dedicated branch/path, but ruleset bypass identities themselves become part of the freshness TCB and must be intentionally narrow. Do not mistake repository locality for independence: independence comes from separate authorization, not a separate filename.

Do not use GitHub's mutable "latest release" marker as this root. GitHub immutable releases lock associated tags and assets after publication and generate release attestations, but GitHub documents that title/notes and whether a release is marked latest can still change. Immutable releases are therefore valuable publication-integrity evidence, not the monotonic current-release authority.

### Historical acceptance, revocation, and emergency rollback

Monotonic policy must describe **authorization epochs**, not force artifact version numbers to increase forever. This separates three operations:

1. Historical verification: verify that publication P was authentic under the policy that authorized it at the time. It may remain cryptographically/historically valid after it stops being current.
2. Current acceptance: require the current trusted policy generation to name P's exact publication-record digest and require its signer/source identities to be non-revoked under that generation.
3. Emergency rollback: never decrement the policy generation. Instead create a new higher generation that deliberately points to a previously published, still-verified publication digest, with an explicit rollback authorization/reason in the reviewed policy change. Example: generation 12 names release B; emergency rollback creates generation 13 naming earlier release A. Consumers see forward-moving policy even though artifact version moves backward.

Revocation follows the same model. Revoking a signer revision or publication for current use advances policy to a new generation that removes that authority; it does not erase historical evidence or rewrite an older generation. If no release is currently safe, policy needs an explicit `NO_CURRENT_RELEASE`/deny state at a new generation rather than silently falling back to an older record. This prevents a freeze attacker from interpreting absence/failure as permission to replay the last cryptographically valid publication.

The exact storage branch/path, reviewer set, ruleset, and update mechanism must be chosen only when the real M7 release workflow and repository permissions are known. No policy file, tag, release, workflow, or speculative regression is created by this audit.
