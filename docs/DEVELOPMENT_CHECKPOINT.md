...
- Concurrent publication is now fail-closed per output path. A concrete race remained where two final builders targeting the same `OUT` could invalidate or overwrite one another's ZIP/sidecars/`.ready` despite each builder being internally fail-closed. `build-final-release.sh` now acquires an atomic `<OUT>.build.lock` before invalidating readiness or touching release output; a losing builder exits without mutating the active builder's state, and the owner releases the lock on all normal/error exits. `test-release-build-lock.sh` verifies both non-interference and lock release after a later failure. Hardening commits `5d315f73c7aaac3c96ebee15b881f7b99a5e92c3`, `262acbd6adb5f828cc982f3e876005740a5ca2a9`, `b31f6f1c3733f4bdc51dcaadd44654ef809f814c`, workflow head `e1cffc75736a5b3aa5620f95247ebb7ea7db84cf`; Route State Contract run `36002946184` SUCCESS passed the lock regression, all release gates/orchestration, repository validation, candidate ZIP build/upload, and post-job cleanup.
- Independent compare from exact candidate through checkpoint head `0159ba5d8d35d1e3bc95bf766d41fe10f9cb9135` reports 175 commits ahead. The hardening changes in this interval remain confined to scripts/workflow/docs; no product `extension/` bytes were edited in this work.

## Remaining path
1. Audit only new concrete cross-boundary identity/publication failure modes; candidate/live/tooling/readiness semantics and same-output concurrent-builder serialization are covered. Do not add source-pattern checks or duplicate already-proven boundaries without a demonstrated failure mode.
