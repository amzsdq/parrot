...
- Release provenance could still misidentify the runtime used to build: `RELEASE_SHA` came from HEAD even if tracked release scripts or release notes had local modifications. `scripts/verify-release-worktree.sh` now rejects any tracked difference from HEAD under `scripts/` or the selected release-notes file before candidate/live gates; external live evidence remains allowed because provenance hashes it separately, and extension dirtiness remains the preflight's responsibility. `scripts/test-release-worktree.sh` executes clean, dirty-tooling, dirty-notes, external-evidence, and extension-scope cases; builder orchestration also verifies this gate fails closed and removes stale outputs. Head `e587d56687a1b168d6cc93efab5c5cf52b85d7ba`; Route State Contract run `35998119144` SUCCESS confirms the regression in CI.
- Two remaining provenance-identity gaps were found. First, the caller could supply an arbitrary candidate SHA; because preflight only compared `extension/` against that caller-selected commit, a non-M5/M6 candidate with matching evidence could be represented as releasable. `verify-release-preflight.sh` now requires the exact M5/M6 candidate `f51e4ba53753dade3bd3f9a64e2b3c50ca05d691`, and its executable regression rejects `HEAD` even when product bytes match. Second, a caller-selected untracked release-notes file could pass `git diff HEAD` while its bytes were absent from `RELEASE_SHA`; `verify-release-worktree.sh` now requires the selected notes path to be tracked, with an executable untracked-notes rejection. Head `b8f4c94862af7ea88d4cc2ba7751295256450609`; Route State Contract run `35998670531` is queued and must be checked before claiming CI confirmation.
- Independent compare from exact M5 candidate through head `b8f4c94862af7ea88d4cc2ba7751295256450609` reports 130 commits ahead and no changed `extension/` files; product bytes remain identical to the M5 candidate.

## Remaining path
1. Check Route State Contract run `35998670531`. If it fails, repair the exact regression and re-run. If it succeeds, continue auditing only material provenance/publishable-state hazards; do not expand low-value source-pattern checks.
