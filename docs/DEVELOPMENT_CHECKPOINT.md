...
- Release-repository attribution now has an external trust anchor at consumption: `verify-release-ready.sh <release.zip> <expected-release-repo-sha>` requires the provenance `release_repo_sha` to equal a trusted expected full commit SHA rather than accepting any syntactically valid 40-hex value. This closes the concrete self-consistent forgery where provenance and `.ready` are recomputed around a false repository SHA. `test-release-ready.sh` exercises that forgery. Route State Contract run `36005052974` SUCCESS, including readiness regression, full orchestration, repository validation, and candidate ZIP build/upload. This deliberately does not claim that an unsigned bundle can prove its own repository origin; the expected SHA must come from a trusted release channel/record.
- Product `extension/` bytes were not edited by this hardening work.

## Remaining path
1. Keep release-repository attribution anchored externally at consumption. Do not try to make the unsigned bundle self-authenticating with more internal hashes; that would not add a trust root. If M7 publication later provides a signed/attested release identity, bind the expected release SHA to that trusted channel rather than duplicating metadata inside the bundle.
2. Continue auditing only genuinely new cross-boundary release identity/publication failures; do not repeat candidate/live/tooling/readiness/concurrency/repository-attribution cases already covered.
3. Obtain a supported authenticated interactive Chrome+ChatGPT surface and execute M6-C2 onward against exact candidate.
4. After M6 PASS, re-verify candidate identity, freeze release-repository SHA, run final builder, persist release evidence, verify readiness against the trusted expected release SHA at consumption, and close M7 only when every criterion passes.
