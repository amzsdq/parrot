...
2. Do not pin either existing CI workflow as the release signer. After M6 PASS, create/review the dedicated release publication workflow under the documented trusted-builder gates, freeze its canonical path, then add `--signer-workflow` and executable wrong-workflow/wrong-source regressions; add `--signer-digest` only if intentionally pinning one immutable workflow revision.
