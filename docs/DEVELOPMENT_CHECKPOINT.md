...
- GitHub's current attestation model confirms two important implementation constraints: verification must cryptographically validate signature/timestamps and repository signer identity, not merely fetch attestation JSON; and attestations are looked up/verified by subject digest. Therefore the publication record itself should be the attested subject (or an equivalent custom predicate must be explicitly verified), rather than assuming an attested ZIP authenticates sidecar identities.
- Pure pre-M6 schema hardening is now executable: `scripts/verify-publication-record.mjs` accepts exactly four unique fields (`schema=parrot-release-publication-v1` plus the three identities), rejects unknown/duplicate/missing fields, schema downgrade, and malformed/lax-case hashes. `scripts/test-publication-record-verifier.mjs` covers those fail-closed cases. Workflow commit `6abcbc29a704df962668b9151299a1cfd8494a7b` adds it to Route State Contract; run `36006849396` was still in progress at checkpoint write and must be confirmed next.
- Product `extension/` bytes were not edited by this hardening work.

## Remaining path
1. Confirm Route State Contract run `36006849396`; fix only an exact executable failure if present.
2. Keep the publication-record verifier as the parser after cryptographic attestation verification. Do not let raw/unverified predicate JSON become trusted input.
3. Before implementing an attestation workflow, pin the expected repository identity (`amzsdq/parrot`) and intended predicate/subject semantics in the consumer path; verify that the attested subject digest is the exact publication-record bytes that are subsequently parsed. This prevents a valid attestation for some other artifact/record from being confused with the trusted release record.
4. Do not create a real release or attestation before M6 PASS. Continue only safe schema/verifier preparation or genuinely new trust-boundary audits.
5. Obtain a supported authenticated interactive Chrome+ChatGPT surface and execute M6-C2 onward against exact candidate.
6. After M6 PASS, re-verify candidate identity, freeze release-repository SHA, run final builder, persist release evidence, create the three-identity publication record, attest that exact record, cryptographically verify repository/signer + subject digest, then pass the parsed identities to `verify-release-ready.sh`.
