...
- Readiness consumption now fails closed after publication too. A `.ready` marker is treated as a hash manifest, not a permanent truth bit: `scripts/verify-release-ready.sh` requires exactly one readiness/hash identity, recomputes ZIP/checksum/file-list/provenance hashes, and rechecks checksum/provenance binding. `scripts/test-release-ready.sh` mutates each component after publication and also corrupts the marker, requiring every such state to become non-consumable. Route State Contract run `36001689948` was queued for head `816e2013d748f43370ce7ce762757e42d7264324` when this checkpoint was written; do not claim this boundary verified until that run succeeds.
- Product `extension/` bytes were not edited by this hardening work.

## Remaining path
1. First resolve Route State Contract run `36001689948`. If it succeeds, continue auditing only material cross-boundary identity/publication hazards. Candidate tree, live evidence, release tooling/notes, artifact/live hashes, stale-output cleanup, readiness creation, and readiness consumption are now frozen/bound; add another hardening boundary only for a concrete failure mode.
