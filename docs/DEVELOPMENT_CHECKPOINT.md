...
- Archive integrity/checksum orchestration is now shared through `scripts/build-release-archive.sh`; production `build-final-release.sh` delegates ZIP creation, `unzip -t`, sorted file-list generation, SHA256 generation/check, and manifest-version assertion to it. `scripts/test-release-archive.sh` executes positive archive/hash/file-list assertions plus `.DS_Store` exclusion, wrong-version rejection, non-ZIP rejection, and tamper checksum rejection. Integration head `e6432f8254288cf4d89dd71a8c0c4eba5eb33d77` triggered Route State Contract run `35996150378`; it was queued at checkpoint time, so do not claim this archive regression CI-confirmed until that run succeeds.
- No product `extension/` bytes were changed by these release-hardening commits.

## Remaining path
1. Confirm Route State Contract run `35996150378`; repair the exact archive regression failure if any.
2. If successful, inspect remaining builder orchestration for the next behavior still protected only by structural/source-pattern assertions and convert the highest-value one to executable shared production/test logic.
