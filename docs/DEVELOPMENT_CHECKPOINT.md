...
- Independent compare from exact M5 candidate through head `e587d56687a1b168d6cc93efab5c5cf52b85d7ba` reports 125 commits ahead and no changed `extension/` files; product bytes remain identical to the M5 candidate.

## Remaining path
1. Continue auditing only material fail-closed release hazards. In particular, check whether any state outside the four release sidecars or any provenance input can still make a failed/local-modified build look like a committed successful release; avoid low-value source-pattern checks.
