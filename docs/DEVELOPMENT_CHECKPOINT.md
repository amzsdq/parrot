...
- Frozen tooling nested escape for live verification was fixed: `verify-candidate-live-evidence.sh` resolves its Node verifier relative to `$0`; corrected wrapper contract run `36003690680` SUCCESS at `6f9a64beb5a71e4c542ed32531d6c5b2838a6f17`.
- Durable-evidence gap is now being closed in production: the exact immutable live-result bytes that passed candidate verification are persisted as `<release>.live-result.txt`; provenance hashes that persisted sidecar, `.ready` binds its hash, cleanup/failure removes it, and `verify-release-ready.sh` rehashes and reruns candidate-bound semantic verification at consumption. `test-release-ready.sh` includes post-publish evidence mutation plus self-consistent invalid-evidence forgery regressions. Boundary alignment commit `fab2a65c03033ae38b7a1c777349d6fbd07c24d3`; Route State Contract run `36003984882` is pending and must pass before this boundary is considered confirmed.
- A second concrete nested-tooling escape was found during the same audit: frozen `verify-release-repository.sh` still invoked `node scripts/verify-repo.mjs` from mutable CWD. It now resolves `verify-repo.mjs` beside `$0`, and `test-release-tooling-snapshot.sh` mutates the worktree verifier to unconditional success while breaking the repository to require the frozen verifier to reject it. Latest regression commit `61cbf8470190d03d29aa848160cf2aec86e973b0`; Route State Contract run `36004031744` is pending.
- Product `extension/` bytes were not edited by this hardening work.

## Remaining path
1. Check latest Route State Contract run for `61cbf8470190d03d29aa848160cf2aec86e973b0`; fix executable failures without weakening durable-evidence or frozen-tooling contracts.
2. Once green, audit whether `release_repo_sha` has a concrete consumer-time semantic gap worth closing. Do not add format/pattern checks without a demonstrated failure mode.
