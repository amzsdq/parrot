...
- Signer-workflow pinning was audited against the current repository and GitHub CLI contract. GitHub recommends `--signer-workflow` (or `--signer-digest`) for stronger actor identity, but the repository currently has only `.github/workflows/m6-chromium-load-smoke.yml` and `.github/workflows/route-state-contract.yml`; neither is an authorized M7 release signer. Therefore adding a guessed signer pin or wrong-signer regression now would encode a false trust root. `docs/TRUSTED_PUBLICATION_RECORD.md` now makes workflow creation/review + canonical path freeze an explicit M7 gate before adding the signer pin/regression (commit `82d2930b756c049bd5eb966d63dfa79ef6c4b25c`).
...
2. Do not pin either existing CI workflow as the release signer. After M6 PASS, create/review the dedicated release publication workflow, freeze its canonical path, then add `--signer-workflow` and an executable wrong-workflow regression; add `--signer-digest` only if intentionally pinning one immutable workflow revision.
